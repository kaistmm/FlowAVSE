...
def visualize_example(mix, estimate, target, orig=False, idx_sample=0, epoch=0, name="", sample_rate=16000, hop_len=128, return_fig=False):
	...
	if return_fig:
		...
		if orig is True:
			save_path = '/mnt/scratch/datasets/spec/long'
		else:
			save_path= '/mnt/scratch/datasets/spec'
		plt.savefig(os.path.join(save_path, f"spectro_{idx_sample}_epoch{epoch}{name}.png"), bbox_inches="tight")
		plt.close()
